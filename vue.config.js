...
const UglifyPlugin = require('uglifyjs-webpack-plugin') //打包时候去掉console.log与debugger调试，防止内存泄漏
...
module.exports = {
    ...
    chainWebpack: config => { //chainWebpack放了loader,更改和添加loader
        ...
        config.module.rule('images')
            ...
            .use('url-loader') //需要cnpm i url-loader -D
            .loader('url-loader')
            .options({
            ...
            .end()
            .test(/\.(png|jpe?g|gif|svg)(\?.*)?$/)
            .use('image-webpack-loader') //需要cnpm i image-webpack-loader -D
            .loader('image-webpack-loader')
            .options({ bypassOnDebug: true })
            .end()

        // 配置Jquery  需要cnpm i jquery -D
            ...
            const optimization = {
                ...
                minimizer: [new UglifyPlugin({
                    uglifyOptions: {
                        warnings: false,
                        compress: {
                            drop_console: true, // true代表去掉console,
                            drop_debugger: false, // 代表不打算去掉debugge
    ...
    // css相关配置
    css: {
        extract: true, // 是否使用css分离插件 ExtractTextPlugin,就是单独提取css
    ...
    devServer: {
        ...
        proxy: {
            '/api': {
                target: 'http://mall-pre.springboot.cn', //target为空的话会报错TypeError: Cannot read property 'upgrade' of undefined
